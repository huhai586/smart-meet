export default {
  // Popup
  "log_captions": "Google मीटिंग कैप्शन लॉग करें",
  "show_captions": "कैप्शन दिखाएं",
  "show_captions_desc": "मुख्य प्रदर्शन देखने के लिए साइडबार खोलें",
  "hide_captions": "कैप्शन छुपाएं",
  "clear_captions": "कैप्शन साफ़ करें",
  "clear_captions_desc": "सभी कैप्शन हटा दिए जाएंगे",
  "translation_language": "अनुवाद भाषा",
  "select_language": "अनुवाद के लिए भाषा चुनें",
  "settings": "सेटिंग्स",
  "settings_desc": "AI कॉन्फ़िगर करें, डेटा सिंक करें और अधिक",
  "clear_confirm": "क्या आप वाकई सभी कैप्शन साफ़ करना चाहते हैं?",

  // Captions
  "translate": "अनुवाद",
  "explain": "व्याख्या",
  "polish": "सुधार",
  "analysis": "विश्लेषण",
  "ask": "पूछें",
  "summary": "सारांश",
  "no_messages": "कोई संदेश नहीं",
  "no_messages_on_date": "{date} को कोई संदेश नहीं",

  // Common
  "on": "चालू",
  "off": "बंद",
  "success": "सफलता",
  "error": "त्रुटि",
  "language_set": "अनुवाद भाषा {language} पर सेट की गई",

  // Settings
  "ai_settings": "AI सेटिंग्स",
  "gemini_api_config": "Gemini API कॉन्फ़िगरेशन",
  "api_key_desc": "AI सुविधाओं को सक्षम करने के लिए अपनी Gemini API कुंजी दर्ज करें। आप Google AI Studio से API कुंजी प्राप्त कर सकते हैं।",
  "api_key_info": "AI कार्यों के लिए आसानी से उपयोग करने के लिए कृपया अपनी Gemini API कुंजी दर्ज करें।",
  "api_key_source": "आप API कुंजी प्राप्त कर सकते हैं",
  "enter_api_key": "Gemini API कुंजी दर्ज करें",
  "save_settings": "सेटिंग्स सहेजें",
  "settings_saved": "सेटिंग्स सफलतापूर्वक सहेजी गईं!",
  "no_api_key": "Gemini API कुंजी नहीं है?",

  // Google Drive Integration
  "google_drive_integration": "Google Drive",
  "google_drive_integration_desc": "Google ड्राइव पर अपने कैप्शन डेटा का बैकअप और पुनर्स्थापना करें",
  "google_drive_access_required": "Google ड्राइव पहुंच आवश्यक है",
  "google_drive_connect_desc": "बैकअप और रिस्टोर सुविधाओं को सक्षम करने के लिए अपने Google खाते से कनेक्ट करें",
  "backup_section": "बैकअप",
  "backup_desc": "मीटिंग डेटा को Google ड्राइव पर बैकअप करें",
  "backup_button": "अभी बैकअप करें",
  "last_backup": "अंतिम बैकअप",
  "restore_section": "पुनर्स्थापित करें",
  "restore_desc": "Google ड्राइव से मीटिंग डेटा पुनर्स्थापित करें",
  "restore_button": "पुनर्स्थापित करें",
  "restore_all": "सभी पुनर्स्थापित करें",
  "refresh": "रीफ्रेश करें",
  "upload": "अपलोड करें",
  "delete": "हटाएं",
  "conflict_title": "विवाद समाधान",
  "overwrite": "ओवरराइट करें",
  "skip": "छोड़ें",
  "always_overwrite": "हमेशा ओवरराइट करें",
  "always_skip": "हमेशा छोड़ें",

  // Export Captions
  "export_captions_text": "कैप्शन टेक्स्ट निर्यात करें",
  "export_captions_desc": "चुनी हुई तारीख के लिए मीटिंग कैप्शन निर्यात करें।",
  "export_captions_button": "TXT के रूप में निर्यात करें",
  "export_as_txt": "TXT के रूप में निर्यात करें",
  "export_as_json": "JSON के रूप में निर्यात करें",
  "no_meeting_data_for_export": "निर्यात के लिए कोई मीटिंग डेटा उपलब्ध नहीं",
  "no_data_found": "चुनी हुई मीटिंग के लिए कोई डेटा नहीं मिला",
  "export_success": "निर्यात सफल",
  "select_meeting_to_export": "निर्यात के लिए मीटिंग चुनें",
  "select_meeting_desc": "कृपया निर्यात के लिए एक मीटिंग चुनें:",
  "export": "निर्यात करें",
  "reset": "रीसेट करें",

  // Font Size Control
  "font_size_control": "फ़ॉन्ट आकार नियंत्रण",
  "font_size_control_desc": "कैप्शन और AI सारांश का फ़ॉन्ट आकार समायोजित करें। प्रत्येक चरण डिफ़ॉल्ट से 1px बदलता है।",
  "captions_font_size": "कैप्शन",
  "summary_font_size": "AI सारांश",

  // Chat History
  "chat_history": "चैट इतिहास",
  "search_content": "चैट सामग्री में खोजें",
  "search_for": "खोजें: {term}",
  "clear": "साफ़ करें",
  "no_matching_content": "कोई मिलान चैट सामग्री नहीं मिली",
  "no_matching_history": "कोई मिलान चैट इतिहास नहीं मिला",
  "no_chat_history": "कोई चैट इतिहास नहीं मिला",
  "chat_records": "चैट रिकॉर्ड - {date}",
  "no_records_for_date": "इस तिथि के लिए कोई चैट रिकॉर्ड नहीं मिला",
  "searching_content": "चैट सामग्री में खोज रहे हैं...",
  "loading_history": "चैट इतिहास लोड हो रहा है...",

  // Sidepanel
  "captions": "कैप्शन",
  "extension": "एक्सटेंशन",
  "translation_records": "अनुवाद",
  "download_captions": "कैप्शन डाउनलोड करें",
  "sidepanel_summary": "AI Summary",
  "sidepanel_settings": "मुख्य इंटरफ़ेस सेटिंग्स",
  "sidepanel_settings_desc": "अपने वर्कफ़्लो के अनुरूप मुख्य इंटरफ़ेस में टैब और बटन की दृश्यता को अनुकूलित करें।",
  "sidepanel_visibility_settings": "टैब दृश्यता सेटिंग्स",
  "sidepanel_visibility_desc": "मुख्य इंटरफ़ेस साइडपैनल में कौन से टैब दिखाई देते हैं, यह कॉन्फ़िगर करें।",
  "captions_tab_desc": "मीटिंग कैप्शन को रीयल-टाइम में देखें और प्रबंधित करें",
  "summary_tab_desc": "AI-संचालित मीटिंग सारांश और अंतर्दृष्टि उत्पन्न करें",
  "translation_tab_desc": "अनुवाद इतिहास और शब्द रिकॉर्ड तक पहुंचें",
  "caption_buttons_visibility_settings": "कैप्शन एक्शन बटन",
  "caption_buttons_visibility_desc": "AI सुविधाओं तक त्वरित पहुंच के लिए प्रत्येक कैप्शन पर दिखाई देने वाले एक्शन बटन को नियंत्रित करें।",
  "translate_button_desc": "कैप्शन टेक्स्ट को अपनी पसंदीदा भाषा में अनुवाद करें",
  "polish_button_desc": "AI के साथ कैप्शन टेक्स्ट को सुधारें और परिष्कृत करें",
  "analysis_button_desc": "कैप्शन से विस्तृत विश्लेषण और अंतर्दृष्टि प्राप्त करें",

  // Sidebar
  "calendar_view": "कैलेंडर दृश्य",
  "configure_assistant": "मीटिंग सहायक कॉन्फ़िगर करें",

  // Summary
  "loading": "लोड हो रहा है",
  "no_meeting_data": "कोई मीटिंग डेटा नहीं",
  "summary_question": "कृपया आज की मीटिंग की सामग्री का सारांश दें",
  "summary_placeholder": "कृपया आज की मीटिंग की सामग्री का सारांश दें",
  "summary_empty_state": "अभी तक कोई सारांश नहीं, शुरू करने के लिए नीचे एक प्रश्न दर्ज करें",
  "search_button": "खोजें",
  "submit_button": "सबमिट करें",

  // Extension
  "extension_settings": "एक्सटेंशन सेटिंग्स",
  "specific_highlight_words": "विशिष्ट हाइलाइट शब्द",
  "specific_highlight_desc": "विशिष्ट शब्द या वाक्यांश जोड़ें जिन्हें आप मीटिंग कैप्शन में हाइलाइट करना चाहते हैं।",
  "add_word": "शब्द जोड़ें",
  "enter_word": "शब्द दर्ज करें",
  "domain_specific_highlights": "डोमेन-विशिष्ट हाइलाइट",
  "domain_specific_desc": "डोमेन-विशिष्ट शब्दावली और कीवर्ड के आधार पर हाइलाइटिंग कॉन्फ़िगर करें।",
  "current_domain_tags": "वर्तमान डोमेन टैग",
  "domain_placeholder": "अपना रुचि का डोमेन दर्ज करें",
  "keywords_placeholder": "उन कीवर्ड का वर्णन करें जिन्हें आप हाइलाइट करना चाहते हैं (जैसे तकनीकी शब्द, उद्योग शब्दावली)",
  "generate_keywords": "कीवर्ड जनरेट करें",
  "preview_keywords": "जनरेट किए गए कीवर्ड का पूर्वावलोकन करें",
  "review_keywords": "अपने डोमेन के लिए जनरेट किए गए कीवर्ड की समीक्षा करें और अनुकूलित करें। किसी कीवर्ड को हटाने के लिए उस पर क्लिक करें।",
  "apply_keywords": "कीवर्ड लागू करें",
  "cancel": "रद्द करें",

  // Words (Translation Records)
  "longman_3000": "लॉन्गमैन 3000",
  "longman_tab_desc": "लॉन्गमैन 3000 सामान्य शब्दावली समीक्षा और अभ्यास",
  "search_words": "शब्द खोजें...",
  "no_words_found": "कोई शब्द नहीं मिला",
  "added_to_starred": "पसंदीदा में जोड़ा गया",
  "removed_from_starred": "पसंदीदा से हटाया गया",
  "starred": "पसंदीदा",
  "total_vocabulary": "कुल शब्दावली",
  "starred_words": "चिह्नित शब्द",
  "all": "सभी",
  "mastered": "निपुण",
  "translation_history": "अनुवाद इतिहास",
  "reset": "रीसेट करें",
  "click_to_translate": "देखने के लिए क्लिक करें",
  "clear_history_confirm": "क्या आप वाकई सभी अनुवाद इतिहास साफ़ करना चाहते हैं?",
  "clear_history_desc": "यह क्रिया पूर्ववत नहीं की जा सकती।",
  "yes_clear_all": "हां, सभी साफ़ करें",
  "no_keep_it": "नहीं, इसे रखें",
  "history_cleared": "अनुवाद इतिहास सफलतापूर्वक साफ़ किया गया",
  "no_history": "अभी तक कोई अनुवाद इतिहास नहीं है। अनुवाद के लिए कैप्शन में किसी शब्द पर क्लिक करें।",

  // Translation Errors
  "translation_service_not_configured": "अनुवाद सेवा कॉन्फ़िगर नहीं है। कृपया अपनी API सेटिंग्स जांचें।",
  "translation_network_error": "नेटवर्क त्रुटि। कृपया अपना इंटरनेट कनेक्शन जांचें और पुनः प्रयास करें।",
  "translation_failed": "अनुवाद असफल",
  "translation_service_unavailable": "अनुवाद सेवा वर्तमान में उपलब्ध नहीं है",
  "unexpected_error": "एक अप्रत्याशित त्रुटि हुई। कृपया पुनः प्रयास करें।",
  "domain_explanation_failed": "डोमेन शब्द की व्याख्या करने में असफल। कृपया अपनी AI सेवा कॉन्फ़िगरेशन जांचें।",

  // AI Error Handler
  "ai_service_configuration_required": "AI सेवा कॉन्फ़िगरेशन आवश्यक है",
  "ai_service_not_configured_message": "AI सेवा कॉन्फ़िगर नहीं है या तैयार नहीं है। AI सुविधाओं का उपयोग करने से पहले कृपया विकल्प पृष्ठ पर जाकर अपनी AI सेवा (OpenAI, Gemini या xAI) को कॉन्फ़िगर करें।",
  "go_to_options": "विकल्प पर जाएं",

  // Clear Captions
  "clear_captions_warning": "चेतावनी: यह कार्रवाई पूर्ववत नहीं की जा सकती",
  "clear_captions_warning_desc": "यह आपके ब्राउज़र संग्रहण से सभी रिकॉर्ड किए गए कैप्शन को स्थायी रूप से हटा देगा।",
  "clear_all_captions": "सभी कैप्शन साफ़ करें",
  "clear_all_data": "सभी डेटा साफ़ करें",
  "confirm_clear_captions": "कैप्शन साफ़ करने की पुष्टि करें",
  "yes_clear_data": "हाँ, डेटा साफ़ करें",
  "data_cleared_success": "डेटा सफलतापूर्वक साफ़ किया गया",

  // UI Language
  "ui_language": "UI भाषा",
  "select_ui_language": "UI भाषा चुनें",
  "ui_language_desc": "Smart Meet के यूजर इंटरफेस के लिए भाषा चुनें",
  "ui_language_set": "UI भाषा {language} पर सेट की गई",

  // Translation Language
  "select_translation_language": "अनुवाद भाषा चुनें",
  "translation_language_desc": "टेक्स्ट सामग्री का अनुवाद करने के लिए भाषा चुनें।",
  "translation_language_set": "अनुवाद भाषा {language} पर सेट की गई",
  "translation_target_language": "लक्ष्य अनुवाद भाषा",
  "translation_target_language_desc": "क्लिक करने पर कैप्शन का अनुवाद करने के लिए भाषा सेट करें",

  // मीटिंग इंटरफेस
  "meeting_interface": "मीटिंग इंटरफेस",
  "meeting_interface_desc": "Google Meet मीटिंग के लिए इंटरफेस तत्वों और नियंत्रणों को कॉन्फ़िगर करें",

  // कैप्शन टॉगल बटन
  "caption_toggle_button": "कैप्शन टॉगल बटन",
  "caption_toggle_button_desc": "Google Meet पेजों पर एक फ्लोटिंग बटन दिखाएं जो कैप्शन को जल्दी छुपाने/दिखाने के लिए है",
  "caption_toggle_enabled": "कैप्शन टॉगल बटन सक्षम किया गया",
  "caption_toggle_disabled": "कैप्शन टॉगल बटन अक्षम किया गया",
  "captions_disabled_title": "कैप्शन बंद हो गए",
  "captions_disabled_body": "यह एक्सटेंशन मीटिंग सामग्री प्राप्त करने के लिए Google Meet कैप्शन पर निर्भर करता है। वर्तमान में कैप्शन बंद हैं — नए कैप्शन कैप्चर नहीं होंगे।\n\nट्रांसक्रिप्ट प्राप्त करना जारी रखने के लिए कैप्शन फिर से चालू करें।",
  "captions_disabled_reenable": "कैप्शन फिर से चालू करें",
  "captions_disabled_dismiss": "समझ गया",

  // Sticker Note
  "sticker_note": "स्टिकर नोट",
  "sticker_note_desc": "त्वरित मीटिंग नोट्स लेने के लिए स्टिकी नोट्स सक्षम करें",
  "sticker_enabled": "स्टिकर नोट सक्षम किया गया",
  "sticker_disabled": "स्टिकर नोट अक्षम किया गया",
  "add_note": "नोट जोड़ें",

  // स्वचालित सिंक सेटिंग्स
  "auto_sync_on_meeting_end": "मीटिंग समाप्त होने पर स्वचालित सिंक",
  "auto_sync_on_meeting_end_desc": "Google Meet कॉल छोड़ने पर स्वचालित रूप से बैकअप लें",
  "auto_sync_on_startup": "ब्राउज़र स्टार्टअप पर स्वचालित सिंक",
  "auto_sync_on_startup_desc": "ब्राउज़र शुरू होने पर हाल के 5 दिनों की चैट हिस्ट्री को स्वचालित रूप से सिंक करें",

  // Google Drive - अतिरिक्त
  "backup_to_drive": "Drive पर बैकअप करें",
  "backup_title": "बैकअप",
  "backup_title_desc": "अपनी चैट हिस्ट्री को Google Drive पर बैकअप करें। मौजूदा फ़ाइलों को अधिलेखित या छोड़ा जा सकता है।",

  // Google Drive - पुनर्स्थापना अनुभाग
  "restore_files": "फ़ाइलें पुनर्स्थापित करें",
  "restore_files_desc": "Google Drive में अपनी बैकअप फ़ाइलों को देखें और प्रबंधित करें",
  "view_details": "विवरण देखें",

  // Google Drive - फ़ाइल सूची
  "backup_date": "बैकअप तिथि",
  "modified": "संशोधित",
  "actions": "क्रियाएं",
  "view_chat_records": "चैट रिकॉर्ड देखें",
  "restore_this_backup": "इस बैकअप को पुनर्स्थापित करें",
  "download_to_local": "स्थानीय रूप से डाउनलोड करें",
  "delete_this_backup": "इस बैकअप को हटाएं",
  "delete_file": "फ़ाइल हटाएं",
  "delete_file_confirm": "क्या आप वाकई \"{fileName}\" को हटाना चाहते हैं?",
  "search_backups": "बैकअप खोजें...",
  "all_time": "सभी समय",
  "this_week": "इस सप्ताह",
  "this_month": "इस महीने",
  "no_matching_backup_files": "कोई मेल खाने वाली बैकअप फ़ाइलें नहीं मिलीं",
  "no_backup_files": "कोई बैकअप फ़ाइलें नहीं मिलीं",
  "items_count": "{count} आइटम",

  // Google Drive - प्रमाणीकरण
  "authenticating_with_google": "Google के साथ प्रमाणीकरण...",
  "authenticating_desc": "कृपया प्रतीक्षा करें जब तक हम आपके Google खाते से सुरक्षित रूप से कनेक्ट करते हैं",
  "logging_in": "लॉग इन हो रहा है...",
  "login_with_google": "Google के साथ लॉगिन करें",

  // कैलेंडर दृश्य
  "chat_history_desc": "तिथि के अनुसार व्यवस्थित मीटिंग चैट इतिहास देखें और खोजें",
  "message": "संदेश",
  "messages": "संदेश",

  // AI सेटिंग्स
  "save_configuration": "कॉन्फ़िगरेशन सहेजें",

  // एक्सटेंशन सेटिंग्स
  "extension_settings_desc": "एक्सटेंशन-विशिष्ट सुविधाओं और व्यवहार को कॉन्फ़िगर करें",
  "select_date": "तिथि चुनें",

  // अनुवाद प्रदाता
  "translation_provider": "अनुवाद प्रदाता",
  "translation_provider_desc": "अनुवाद सेवा प्रदाता चुनें",
  "provider_google": "Google Translate",
  "provider_microsoft": "Microsoft अनुवादक",
  "provider_ai": "AI अनुवाद",
  "provider_google_desc": "मुफ़्‍त, कोई कॉन्फ़िगरेशन आवश्यक नहीं",
  "provider_microsoft_desc": "मुफ़्‍त, कोई कॉन्फ़िगरेशन आवश्यक नहीं",
  "provider_ai_desc": "आपकी कॉन्फ़िगर की गई AI सेवा का उपयोग करता है",
  "provider_deepl": "DeepL",
  "provider_deepl_desc": "उच्च गुणवत्ता वाले अनुवाद, प्रमाणीकरण कुंजी आवश्यक है",
  "provider_local": "स्थानीय AI अनुवाद",
  "provider_local_desc": "Chrome अंतर्निहित AI, इंटरनेट की आवश्यकता नहीं",
  "local_translator_title": "स्थानीय AI अनुवादक",
  "local_translator_checking": "उपलब्धता जाँच रहा है...",
  "local_translator_not_supported": "इस डिवाइस पर समर्थित नहीं",
  "local_translator_not_supported_hint": "GPU वाले डेस्कटॉप पर Chrome 138+ आवश्यक है",
  "local_translator_downloading": "अनुवाद मॉडल डाउनलोड हो रहा है...",
  "local_translator_download_progress": "{percent}% डाउनलोड हुआ",
  "local_translator_ready": "मॉडल तैयार है",
  "local_translator_test_title": "अनुवाद परीक्षण",
  "local_translator_input_placeholder": "अनुवाद के लिए टेक्स्ट दर्ज करें...",
  "local_translator_output_placeholder": "अनुवाद यहाँ दिखाई देगा...",
  "local_translator_translate_btn": "अनुवाद करें",
  "local_translator_done": "स्थानीय AI उपयोग करें",
  "local_translator_source_lang": "स्रोत",
  "deepl_settings": "DeepL सेटिंग्स",
  "deepl_auth_key": "प्रमाणीकरण कुंजी",
  "deepl_auth_key_placeholder": "DeepL प्रमाणीकरण कुंजी दर्ज करें",
  "deepl_auth_key_help": "deepl.com पर अपनी कुंजी प्राप्त करें",
  "deepl_settings_saved": "DeepL सेटिंग्स सहेजी गईं",
  "deepl_auth_key_required": "प्रमाणीकरण कुंजी आवश्यक है",
  "save": "सहेजें",
  "translation_provider_set": "अनुवाद प्रदाता {provider} पर सेट किया गया",
  // AI Settings - Provider UI
  "configuration_removed": "कॉन्फ़िगरेशन हटाया गया",
  "active_service_changed": "डिफ़ॉल्ट सेवा बदली गई",
  "optional": "वैकल्पिक",
  "add_custom_provider": "कस्टम प्रदाता जोड़ें",
  "add": "जोड़ें",
  "provider_name": "प्रदाता नाम",
  "api_base_url": "API आधार URL",
  "openai_compatible_hint": "OpenAI-संगत एंडपॉइंट होना चाहिए",
  "default_model": "डिफ़ॉल्ट मॉडल",
  "search_providers": "प्रदाता खोजें...",
  "select_provider_hint": "कॉन्फ़िगर करने के लिए एक प्रदाता चुनें",
  "select_provider_desc": "बाईं सूची से चुनें",
  "set_as_default": "डिफ़ॉल्ट सेवा के रूप में सेट करें",
  "default_service_desc": "सभी AI सुविधाओं के लिए इस प्रदाता का उपयोग करें",
  "configuration": "कॉन्फ़िगरेशन",
  "get_api_key": "API कुंजी प्राप्त करें",
  "model": "मॉडल",
  "select_model": "एक मॉडल चुनें",
  "models_from_cache": "कैश से मॉडल सूची · हर 24 घंटे में अपडेट",
  "base_url": "आधार URL",
  "base_url_hint": "डिफ़ॉल्ट API एंडपॉइंट ओवरराइड करें (जैसे प्रॉक्सी के लिए)",
  "testing": "कनेक्शन परीक्षण हो रहा है...",
  "connection_success": "कनेक्शन सफल",
  "test_connection": "कनेक्शन परीक्षण",
  "remove": "हटाएं",
  "category_international": "अंतर्राष्ट्रीय",
  "category_chinese": "चीनी",
  "category_local": "स्थानीय",
  "category_custom": "कस्टम",
  "active_badge": "सक्रिय",

  // Summary message UI
  "view_more": "विवरण देखें",
  "view_less": "विवरण छुपाएं",
  "error_request_failed": "अनुरोध विफल",
  "error_status": "त्रुटि",
  "time_just_now": "अभी",
  "time_minutes_ago": "{n} मिनट पहले",
  "time_hours_ago": "{n} घंटे पहले",
  "error_expand": "अनुरोध विफल · विवरण देखें",
  "error_collapse": "विवरण छुपाएं",
  "error_retry": "पुनः प्रयास",
  "error_details": "अधिक जानकारी",
  "err_title_quota": "आज की सीमा समाप्त",
  "err_desc_quota": "आपने आज का मुफ़्त कोटा उपयोग कर लिया है। कल पुनः प्रयास करें या प्लान अपग्रेड करें।",
  "err_action_quota": "प्लान देखें",
  "err_title_auth": "API कुंजी अमान्य",
  "err_desc_auth": "आपकी API कुंजी अमान्य या समाप्त लगती है। अपनी सेटिंग्स जाँचें।",
  "err_title_permission": "प्रवेश अस्वीकृत",
  "err_desc_permission": "आपके खाते को इस मॉडल की अनुमति नहीं है। अपना प्लान जाँचें।",
  "err_title_network": "कनेक्शन विफल",
  "err_desc_network": "AI सेवा से कनेक्ट नहीं हो सका। अपना नेटवर्क जाँचें और पुनः प्रयास करें।",
  "err_title_model": "मॉडल उपलब्ध नहीं",
  "err_desc_model": "चुना हुआ मॉडल नहीं मिला। सेटिंग्स में दूसरा मॉडल चुनें।",
  "err_title_generic": "कुछ गलत हुआ",
  "err_desc_generic": "एक अप्रत्याशित त्रुटि हुई। कृपया पुनः प्रयास करें।",

  // Custom Prompts
  "custom_prompts": "कस्टम प्रॉम्प्ट",
  "custom_prompts_desc": "AI सारांश के लिए पुनः उपयोग योग्य प्रॉम्प्ट टेम्पलेट बनाएं।",
  "custom_prompts_empty": "अभी कोई प्रॉम्प्ट नहीं। + पर क्लिक करें।",
  "go_to_extension_settings": "प्रॉम्प्ट जोड़ने के लिए सेटिंग → एक्सटेंशन पर जाएं।",
  "add_prompt": "प्रॉम्प्ट जोड़ें",
  "edit_prompt": "प्रॉम्प्ट संपादित करें",
  "prompt_title": "शीर्षक",
  "prompt_title_placeholder": "जैसे: उन्नत शब्दावली निकालें",
  "prompt_content": "प्रॉम्प्ट सामग्री",
  "prompt_content_placeholder": "प्रॉम्प्ट टेक्स्ट दर्ज करें…",
  "select_prompt": "प्रॉम्प्ट चुनें",
  "apply_prompts": "लागू करें ({n} चयनित)",
  "save": "सहेजें",

  // AI & Translation Tab
  "tab_ai_translation": "AI सेटिंग्स",
  "tab_ai_translation_desc": "AI सेवा प्रदाता, कस्टम प्रॉम्प्ट और अनुवाद सेटिंग्स",
  "tab_ai_models": "सेवा मॉडल",
  "tab_ai_prompts": "प्रॉम्प्ट लाइब्रेरी",
  "tab_translation_prefs": "अनुवाद प्राथमिकताएं",
  "tab_general": "सामान्य",
  "tab_general_desc": "भाषा, फ़ॉन्ट आकार, सहायता और डेटा प्रबंधन",
  "tab_interface": "इंटरफ़ेस",
  "tab_interface_desc": "साइडपैनल टैब, कैप्शन बटन, मीटिंग ओवरले और हाइलाइट शब्द",
  "tab_history": "कैलेंडर",
  "tab_history_desc": "मीटिंग इतिहास, AI सहायक और निर्यात",
  "tab_cloud_sync": "क्लाउड",
  "tab_cloud_sync_desc": "Google Drive बैकअप और पुनर्स्थापना",
  "meeting_history_tab": "मीटिंग इतिहास",
  "ai_assistant_tab": "AI सहायक",
  "ai_chat_title": "AI मीटिंग सहायक",
  "ai_chat_empty_desc": "उस अवधि की मीटिंग रिकॉर्ड के आधार पर बातचीत शुरू करने के लिए एक तिथि सीमा चुनें",
  "ai_reading_records": "मीटिंग रिकॉर्ड पढ़ा जा रहा है",
  "ai_found_records": "{count} मिले",

  // Popup (additional)
  "extension_name": "Smart Meet",
  "popup_subtitle": "Google Meet एन्हांसमेंट टूल",
  "main_controls": "मुख्य नियंत्रण",
  "quick_actions": "त्वरित क्रियाएं",
  "recommended": "अनुशंसित",

  // AI Chat Panel - Date Range & Input
  "select_date_range": "मीटिंग तिथि सीमा चुनें",
  "preset_3d": "पिछले 3 दिन",
  "preset_7d": "पिछला 1 सप्ताह",
  "preset_14d": "पिछले 2 सप्ताह",
  "preset_1m": "पिछला 1 महीना",
  "preset_2m": "पिछले 2 महीने",
  "preset_3m": "पिछले 3 महीने",
  "preset_custom": "कस्टम सीमा",
  "date_format_short": "D MMM",
  "capsule_text": "{start} - {end} ({count} रिकॉर्ड)",
  "clear_chat": "चैट साफ़ करें",
  "no_ai_provider_before": "कोई AI सेवा कॉन्फ़िगर नहीं है, कृपया",
  "no_ai_provider_link": "AI सेटिंग्स पर जाएं",
  "no_ai_provider_after": "प्रदाता कॉन्फ़िगर करने के लिए।",
  "placeholder_no_date": "पहले तिथि सीमा चुनें...",
  "placeholder_no_provider": "पहले AI सेवा कॉन्फ़िगर करें...",
  "ai_input_placeholder": "प्रश्न दर्ज करें, भेजने के लिए Cmd+Enter या Ctrl+Enter दबाएं",
  "select_ai_service": "AI सेवा चुनें",
  "default_label": "डिफ़ॉल्ट",
  "current_provider": "वर्तमान: {name}",
  "select_ai": "AI चुनें",

  // Shared UI (General tab)
  "help_and_guide": "सहायता और मार्गदर्शिका",
  "open_welcome_guide": "स्वागत मार्गदर्शिका खोलें",
  "ui_language_selector_desc": "एक्सटेंशन इंटरफेस की भाषा चुनें",
  "data_management": "डेटा प्रबंधन",
  "highlight_words_section": "कैप्शन हाइलाइट्स",

  // Options tabs
  "tab_ai_models_nav": "AI मॉडल",
  "tab_visibility": "दृश्यता",
  "tab_translation": "अनुवाद",
  "tab_translation_nav": "अनुवाद",
  "tab_other": "अन्य",
  "tab_calendar": "Google Calendar",
  "tab_data_sync": "मीटिंग इतिहास",
  "tab_google_drive": "Google Drive",
  // Extension section
  "extension_section": "एक्सटेंशन",
  "enable_extension": "एक्सटेंशन सक्षम करें",
  "extension_active_desc": "Google Meet में कैप्शन कैप्चर सक्रिय है",
  "extension_paused_desc": "एक्सटेंशन रुका हुआ है — कोई कैप्शन कैप्चर नहीं होगा",
  "extension_toggle_footer": "एक्सटेंशन अनइंस्टॉल किए बिना कैप्शन कैप्चर को रोकने या फिर शुरू करने के लिए इसे टॉगल करें।",

  // Popup tabs
  "popup_tab_meetings": "मीटिंग",
  "popup_tab_live": "प्रगति में",
  "popup_tab_ai": "AI",
  "popup_tab_settings": "सेटिंग्स",
  "ai_no_content_title": "अभी तक कोई मीटिंग सामग्री नहीं",
  "ai_no_content_sub": "आपकी मीटिंग सारांश और AI अंतर्दृष्टि यहाँ आपकी मीटिंग के बाद दिखाई देंगी।",
  "ai_whats_coming": "आपको क्या मिलेगा",
  "ai_feature_summaries": "AI-जनित सारांश",
  "ai_feature_highlights": "मुख्य बिंदु और कार्य आइटम",
  "ai_feature_ask_meetings": "अपनी मीटिंग के बारे में कुछ भी पूछें",
  "ai_recent_meetings": "हालिया मीटिंग",
  "ai_ask_ai_btn": "AI से पूछें",
  "ai_view_all_meetings": "सभी मीटिंग और सारांश देखें",
  "meetings_connect_title": "अपनी आगामी Google Meet मीटिंग देखें",
  "meetings_connect_sub": "आगामी मीटिंग देखने और रिमाइंडर प्राप्त करने के लिए Google Calendar कनेक्ट करें।",
  "meetings_connect_note": "🔒 केवल-पढ़ने की पहुँच। आपका डेटा निजी है।",
  "meetings_no_upcoming_title": "कोई आगामी मीटिंग नहीं",
  "meetings_no_upcoming_sub": "आपका कैलेंडर खाली है। अपने दिन का आनंद लें!",
  "meetings_open_calendar_settings": "कैलेंडर सेटिंग खोलें",
  "meetings_today_label": "आज · {date}",
  "meetings_synced_now": "अभी सिंक हुआ",
  "meetings_sync_ago": "{time} पहले सिंक हुआ",
  "meetings_sync_h": "घं",
  "meetings_sync_m": "मि",
  "meetings_sync_s": "से",
  "meetings_in_progress": "प्रगति में",
  "meetings_starts_in": "{min} मिनट में शुरू",
  "meetings_untitled": "बिना शीर्षक की मीटिंग",
  "meetings_participants": "{n} प्रतिभागी",
  "meetings_join_btn": "मीटिंग में शामिल हों",
  "meetings_later_today": "आज बाद में",
  "meetings_view_schedule": "पूरा शेड्यूल देखें",
  "live_no_meeting_title": "कोई लाइव मीटिंग नहीं मिली",
  "live_no_meeting_sub": "लाइव कैप्शन और अनुवाद स्थिति देखने के लिए Google Meet मीटिंग में शामिल हों।",
  "live_how_it_works": "यह कैसे काम करता है",
  "live_how_body": "हम स्वचालित रूप से पता लगाएंगे कि आप Google Meet में कब हैं और कैप्शन कैप्चर करना शुरू करेंगे।",
  "live_meet_detected": "Google Meet का पता चला",
  "live_now": "अभी लाइव",
  "live_captions_label": "कैप्शन",
  "live_active": "सक्रिय",
  "live_translation_label": "अनुवाद",
  "live_translation_active": "सक्रिय (EN → {lang})",
  "live_off": "बंद",
  "live_open_panel_btn": "लाइव पैनल खोलें",
  "live_hint": "सेटिंग में अनुवाद प्रबंधित करें",
  "live_manage_settings": "सेटिंग में प्रबंधित करें ›",

  // Settings tab
  "settings_open_full": "पूर्ण सेटिंग्स खोलें",
  "settings_connected": "जुड़ा हुआ",
  "settings_not_connected": "डिस्कनेक्ट",
  "settings_sync_range": "सिंक सीमा",

  // Google Calendar page
  "calendar_page_title": "कैलेंडर",
  "calendar_page_desc": "मीटिंग शेड्यूल प्राप्त करने और रिमाइंडर पाने के लिए अपना Google Calendar सिंक करें",
  "calendar_connection_status": "Google Calendar कनेक्शन स्थिति",
  "calendar_not_connected": "कनेक्ट नहीं है",
  "calendar_not_connected_desc": "आगामी मीटिंग देखने के लिए अपना Google Calendar कनेक्ट करें",
  "calendar_connected": "कनेक्टेड",
  "calendar_connect_button": "Google Calendar कनेक्ट करें",
  "calendar_disconnect_button": "डिस्कनेक्ट करें",
  "calendar_privacy_note": "हम केवल आपके कैलेंडर इवेंट पढ़ते हैं। हम कोई सामग्री संशोधित, बनाएंगे या हटाएंगे नहीं",
  "calendar_learn_more": "अधिक जानें",
  "calendar_sync_settings": "सिंक सेटिंग्स",
  "calendar_auto_sync": "स्वचालित सिंक",
  "calendar_auto_sync_desc": "Google Calendar से नवीनतम मीटिंग समय-समय पर सिंक करें",
  "calendar_sync_frequency": "सिंक आवृत्ति",
  "calendar_sync_frequency_desc": "निर्धारित अंतराल पर स्वचालित सिंक",
  "calendar_sync_range": "सिंक सीमा",
  "calendar_sync_range_desc": "केवल निर्दिष्ट भविष्य की सीमा के भीतर मीटिंग सिंक करें",
  "calendar_sync_content": "सिंक करने की सामग्री",
  "calendar_content_title": "मीटिंग शीर्षक",
  "calendar_content_time": "शुरुआत और अंत समय",
  "calendar_content_link": "Google Meet लिंक",
  "calendar_content_attendees": "उपस्थित लोगों की जानकारी",
  "calendar_content_location": "मीटिंग स्थान",
  "calendar_tip": "टिप: कनेक्ट करने के बाद, आगामी मीटिंग से पहले सिस्टम नोटिफिकेशन मिलेंगे",
  "calendar_frequency_15min": "15 मिनट",
  "calendar_frequency_30min": "30 मिनट",
  "calendar_frequency_1hour": "1 घंटा",
  "calendar_frequency_2hours": "2 घंटे",
  "calendar_range_7days": "अगले 7 दिन",
  "calendar_range_14days": "अगले 14 दिन",
  "calendar_range_30days": "अगले 30 दिन",
  "calendar_range_60days": "अगले 60 दिन",
  "calendar_reminders": "मीटिंग रिमाइंडर",
  "calendar_reminders_enable": "मीटिंग रिमाइंडर सक्षम करें",
  "calendar_reminders_enable_desc": "मीटिंग शुरू होने से पहले सिस्टम नोटिफिकेशन भेजें",
  "calendar_reminders_advance": "अग्रिम रिमाइंडर समय",
  "calendar_reminders_advance_desc": "मीटिंग से कितने मिनट पहले नोटिफिकेशन भेजना है",
  "calendar_reminder_on_time": "मीटिंग समय पर",
  "calendar_reminder_5min": "5 मिनट पहले",
  "calendar_reminder_10min": "10 मिनट पहले",
  "calendar_reminder_15min": "15 मिनट पहले",
  "calendar_reminder_30min": "30 मिनट पहले",
  "calendar_test_notif": "टेस्ट नोटिफिकेशन",
  "calendar_test_notif_desc": "नोटिफिकेशन काम करता है या नहीं, यह जांचने के लिए तुरंत एक टेस्ट रिमाइंडर भेजें",
  "calendar_test_notif_btn": "टेस्ट भेजें",
  "calendar_test_notif_success": "टेस्ट नोटिफिकेशन भेजा गया, सिस्टम नोटिफिकेशन जांचें",
  "calendar_test_notif_fail": "भेजने में विफल, कृपया नोटिफिकेशन अनुमति की पुष्टि करें",
  "calendar_sync_status": "सिंक स्थिति",
  "calendar_last_sync": "अंतिम सिंक: {time}",
  "calendar_sync_now": "अभी सिंक करें",
  "calendar_syncing": "सिंक हो रहा है...",
  "calendar_sync_success": "सिंक सफल, <strong>{n}</strong> कैलेंडर इवेंट प्राप्त हुए",
  "calendar_sync_error_default": "सिंक विफल, कृपया पुनः प्रयास करें",
  "calendar_syncing_msg": "Google Calendar से इवेंट प्राप्त किए जा रहे हैं...",
  "calendar_event_no_title": "(कोई शीर्षक नहीं)",
  "calendar_event_attendees": "{n} उपस्थित",
  "calendar_meet_link": "Meet लिंक",
  "calendar_event_more": "{n} और इवेंट",
  "calendar_sync_empty": "आगामी मीटिंग प्राप्त करने के लिए अभी सिंक करें क्लिक करें",
  "settings_local_data": "स्थानीय डेटा",
  "settings_local_days": "{n} दिन",

  "version_label": "v",
  "sidepanel_disabled_title": "एक्सटेंशन अक्षम है",
  "sidepanel_disabled_desc": "एक्सटेंशन बंद होने के कारण कैप्शन प्राप्त नहीं हो सकते।",
  "sidepanel_disabled_enable": "चालू करें",
  "sidepanel_disabled_ignore": "अनदेखा करें",
  "notif_reminder_title": "मीटिंग रिमाइंडर",
  "notif_starting_now": "मीटिंग अभी शुरू हो रही है",
  "notif_starts_in": "{n} मिनट में शुरू होगी",
  "notif_start_time": "शुरुआत का समय: {time}",
  "notif_click_to_join": "मीटिंग में शामिल होने के लिए क्लिक करें",
  "notif_join_btn": "मीटिंग में शामिल हों",
  "notif_test_title": "📅 टेस्ट मीटिंग रिमाइंडर",
};
