export default {
  // Popup
  "log_captions": "Ghi lại phụ đề cuộc họp Google",
  "show_captions": "Hiển thị phụ đề",
  "show_captions_desc": "Mở thanh bên để xem màn hình chính",
  "hide_captions": "Ẩn phụ đề",
  "clear_captions": "Xóa phụ đề",
  "clear_captions_desc": "Tất cả phụ đề sẽ bị xóa",
  "translation_language": "Ngôn ngữ dịch",
  "select_language": "Chọn ngôn ngữ để dịch",
  "settings": "Cài đặt",
  "settings_desc": "Cấu hình AI, đồng bộ hóa dữ liệu và nhiều hơn nữa",
  "clear_confirm": "Bạn có chắc chắn muốn xóa tất cả phụ đề?",

  // Captions
  "translate": "Dịch",
  "explain": "Giải thích",
  "polish": "Chỉnh sửa",
  "analysis": "Phân tích",
  "ask": "Hỏi",
  "summary": "Tóm tắt",
  "no_messages": "Không có tin nhắn",
  "no_messages_on_date": "Không có tin nhắn vào {date}",

  // Common
  "on": "Bật",
  "off": "Tắt",
  "success": "Thành công",
  "error": "Lỗi",
  "language_set": "Đã đặt ngôn ngữ dịch thành {language}",

  // Settings
  "ai_settings": "Cài đặt AI",
  "gemini_api_config": "Cấu hình Gemini API",
  "api_key_desc": "Nhập khóa Gemini API của bạn để kích hoạt các tính năng AI. Bạn có thể lấy khóa API từ Google AI Studio.",
  "api_key_info": "Vui lòng nhập khóa Gemini API của bạn để sử dụng dễ dàng cho các tác vụ AI.",
  "api_key_source": "Bạn có thể lấy khóa API từ",
  "enter_api_key": "Nhập khóa Gemini API",
  "save_settings": "Lưu cài đặt",
  "settings_saved": "Đã lưu cài đặt thành công!",
  "no_api_key": "Không có khóa Gemini API?",

  // Google Drive Integration
  "google_drive_integration": "Tích hợp Google Drive",
  "google_drive_integration_desc": "Sao lưu và khôi phục dữ liệu phụ đề của bạn vào Google Drive",
  "google_drive_access_required": "Cần quyền truy cập Google Drive",
  "google_drive_connect_desc": "Kết nối với tài khoản Google của bạn để kích hoạt các tính năng sao lưu và khôi phục",
  "backup_section": "Sao lưu",
  "backup_desc": "Sao lưu dữ liệu cuộc họp lên Google Drive",
  "backup_button": "Sao lưu ngay",
  "last_backup": "Sao lưu gần nhất",
  "restore_section": "Khôi phục",
  "restore_desc": "Khôi phục dữ liệu cuộc họp từ Google Drive",
  "restore_button": "Khôi phục",
  "restore_all": "Khôi phục tất cả",
  "refresh": "Làm mới",
  "upload": "Tải lên",
  "delete": "Xóa",
  "conflict_title": "Giải quyết xung đột",
  "overwrite": "Ghi đè",
  "skip": "Bỏ qua",
  "always_overwrite": "Luôn ghi đè",
  "always_skip": "Luôn bỏ qua",

  // Export Captions
  "export_captions_text": "Xuất văn bản phụ đề",
  "export_captions_desc": "Xuất phụ đề cuộc họp cho ngày đã chọn.",
  "export_captions_button": "Xuất dạng TXT",
  "export_as_txt": "Xuất dạng TXT",
  "export_as_json": "Xuất dạng JSON",
  "no_meeting_data_for_export": "Không có dữ liệu cuộc họp để xuất",
  "no_data_found": "Không tìm thấy dữ liệu cho cuộc họp đã chọn",
  "export_success": "Xuất thành công",
  "select_meeting_to_export": "Chọn cuộc họp để xuất",
  "select_meeting_desc": "Vui lòng chọn một cuộc họp để xuất:",
  "export": "Xuất",
  "reset": "Đặt lại",

  // Font Size Control
  "font_size_control": "Điều chỉnh cỡ chữ",
  "font_size_control_desc": "Điều chỉnh cỡ chữ của phụ đề và tóm tắt AI riêng biệt. Mỗi bước thay đổi 1px so với mặc định.",
  "captions_font_size": "Phụ đề",
  "summary_font_size": "Tóm tắt AI",

  // Chat History
  "chat_history": "Lịch sử trò chuyện",
  "search_content": "Tìm kiếm trong nội dung trò chuyện",
  "search_for": "Tìm kiếm: {term}",
  "clear": "Xóa",
  "no_matching_content": "Không tìm thấy nội dung trò chuyện phù hợp",
  "no_matching_history": "Không tìm thấy lịch sử trò chuyện phù hợp",
  "no_chat_history": "Không tìm thấy lịch sử trò chuyện",
  "chat_records": "Bản ghi trò chuyện - {date}",
  "no_records_for_date": "Không tìm thấy bản ghi trò chuyện cho ngày này",
  "searching_content": "Đang tìm kiếm trong nội dung trò chuyện...",
  "loading_history": "Đang tải lịch sử trò chuyện...",

  // Sidepanel
  "captions": "Phụ đề",
  "extension": "Tiện ích mở rộng",
  "translation_records": "Bản ghi dịch",
  "download_captions": "Tải xuống phụ đề",
  "sidepanel_summary": "AI Summary",
  "sidepanel_settings": "Cài đặt Giao diện Chính",
  "sidepanel_settings_desc": "Tùy chỉnh khả năng hiển thị của các tab và nút trong giao diện chính để phù hợp với quy trình làm việc của bạn.",
  "sidepanel_visibility_settings": "Cài đặt Hiển thị Tab",
  "sidepanel_visibility_desc": "Cấu hình các tab hiển thị trong bảng điều khiển bên của giao diện chính.",
  "captions_tab_desc": "Xem và quản lý phụ đề cuộc họp theo thời gian thực",
  "summary_tab_desc": "Tạo tóm tắt và thông tin chi tiết cuộc họp bằng AI",
  "translation_tab_desc": "Truy cập lịch sử dịch và bản ghi từ",
  "caption_buttons_visibility_settings": "Nút Hành động Phụ đề",
  "caption_buttons_visibility_desc": "Kiểm soát các nút hành động xuất hiện trên mỗi phụ đề để truy cập nhanh các tính năng AI.",
  "translate_button_desc": "Dịch văn bản phụ đề sang ngôn ngữ ưa thích của bạn",
  "polish_button_desc": "Cải thiện và tinh chỉnh văn bản phụ đề bằng AI",
  "analysis_button_desc": "Nhận phân tích chi tiết và thông tin chi tiết từ phụ đề",

  // Sidebar
  "calendar_view": "Xem lịch",
  "configure_assistant": "Cấu hình trợ lý cuộc họp",

  // Summary
  "loading": "Đang tải",
  "no_meeting_data": "Không có dữ liệu cuộc họp",
  "summary_question": "Vui lòng tóm tắt nội dung cuộc họp hôm nay",
  "summary_placeholder": "Vui lòng tóm tắt nội dung cuộc họp hôm nay",
  "summary_empty_state": "Chưa có tóm tắt, nhập câu hỏi bên dưới để bắt đầu",
  "search_button": "Tìm kiếm",
  "submit_button": "Gửi",

  // Extension
  "extension_settings": "Cài Đặt Tiện Ích Mở Rộng",
  "specific_highlight_words": "Từ đánh dấu cụ thể",
  "specific_highlight_desc": "Thêm các từ hoặc cụm từ cụ thể mà bạn muốn đánh dấu trong phụ đề cuộc họp.",
  "add_word": "Thêm từ",
  "enter_word": "Nhập từ",
  "domain_specific_highlights": "Đánh dấu theo lĩnh vực",
  "domain_specific_desc": "Cấu hình đánh dấu dựa trên thuật ngữ và từ khóa theo lĩnh vực cụ thể.",
  "current_domain_tags": "Thẻ lĩnh vực hiện tại",
  "domain_placeholder": "Nhập lĩnh vực bạn quan tâm",
  "keywords_placeholder": "Mô tả các từ khóa bạn muốn đánh dấu (ví dụ: thuật ngữ kỹ thuật, thuật ngữ ngành)",
  "generate_keywords": "Tạo từ khóa",
  "preview_keywords": "Xem trước từ khóa đã tạo",
  "review_keywords": "Xem xét và tùy chỉnh các từ khóa đã tạo cho lĩnh vực của bạn. Nhấp vào bất kỳ từ khóa nào để xóa nó.",
  "apply_keywords": "Áp dụng từ khóa",
  "cancel": "Hủy",

  // Words (Translation Records)
  "longman_3000": "Longman 3000",
  "longman_tab_desc": "Xem lại và luyện tập từ vựng thông dụng Longman 3000",
  "search_words": "Tìm kiếm từ...",
  "no_words_found": "Không tìm thấy từ nào",
  "added_to_starred": "Đã thêm vào mục đánh dấu",
  "removed_from_starred": "Đã xóa khỏi mục đánh dấu",
  "starred": "Đã đánh dấu",
  "total_vocabulary": "Tổng số từ",
  "starred_words": "Số từ đã đánh dấu",
  "all": "Tất cả",
  "mastered": "Đã thành thạo",
  "translation_history": "Lịch sử dịch",
  "reset": "Đặt lại",
  "click_to_translate": "Nhấp để hiển thị",
  "clear_history_confirm": "Bạn có chắc chắn muốn xóa tất cả lịch sử dịch?",
  "clear_history_desc": "Hành động này không thể hoàn tác.",
  "yes_clear_all": "Có, xóa tất cả",
  "no_keep_it": "Không, giữ lại",
  "history_cleared": "Đã xóa lịch sử dịch thành công",
  "no_history": "Chưa có lịch sử dịch. Nhấp vào bất kỳ từ nào trong phụ đề để dịch.",

  // Translation Errors
  "translation_service_not_configured": "Dịch vụ dịch thuật chưa được cấu hình. Vui lòng kiểm tra cài đặt API của bạn.",
  "translation_network_error": "Lỗi mạng. Vui lòng kiểm tra kết nối internet và thử lại.",
  "translation_failed": "Dịch thuật thất bại",
  "translation_service_unavailable": "Dịch vụ dịch thuật hiện không khả dụng",
  "unexpected_error": "Đã xảy ra lỗi không mong muốn. Vui lòng thử lại.",
  "domain_explanation_failed": "Không thể giải thích thuật ngữ miền. Vui lòng kiểm tra cấu hình dịch vụ AI của bạn.",

  // AI Error Handler
  "ai_service_configuration_required": "Cần cấu hình dịch vụ AI",
  "ai_service_not_configured_message": "Dịch vụ AI chưa được cấu hình hoặc chưa sẵn sàng. Vui lòng đi đến trang Tùy chọn để cấu hình dịch vụ AI của bạn (OpenAI, Gemini hoặc xAI) trước khi sử dụng các tính năng AI.",
  "go_to_options": "Đi đến Tùy chọn",

  // Clear Captions
  "clear_captions_warning": "Cảnh báo: Hành động này không thể hoàn tác",
  "clear_captions_warning_desc": "Thao tác này sẽ xóa vĩnh viễn tất cả phụ đề đã ghi khỏi bộ nhớ trình duyệt của bạn.",
  "clear_all_captions": "Xóa tất cả phụ đề",
  "clear_all_data": "Xóa tất cả dữ liệu",
  "confirm_clear_captions": "Xác nhận xóa phụ đề",
  "yes_clear_data": "Có, Xóa Dữ Liệu",
  "data_cleared_success": "Dữ liệu đã được xóa thành công",

  // UI Language
  "ui_language": "Ngôn Ngữ Giao Diện",
  "select_ui_language": "Chọn Ngôn Ngữ Giao Diện",
  "ui_language_desc": "Chọn ngôn ngữ cho giao diện người dùng của Smart Meet",
  "ui_language_set": "Ngôn ngữ giao diện đã được đặt thành {language}",

  // Translation Language
  "select_translation_language": "Chọn Ngôn Ngữ Dịch Thuật",
  "translation_language_desc": "Chọn ngôn ngữ để dịch nội dung văn bản.",
  "translation_language_set": "Ngôn ngữ dịch thuật đã được đặt thành {language}",
  "translation_target_language": "Ngôn Ngữ Dịch Đích",
  "translation_target_language_desc": "Đặt ngôn ngữ để dịch phụ đề khi nhấp chuột",

  // Giao diện cuộc họp
  "meeting_interface": "Giao diện cuộc họp",
  "meeting_interface_desc": "Cấu hình các thành phần giao diện và điều khiển cho cuộc họp Google Meet",

  // Nút chuyển đổi phụ đề
  "caption_toggle_button": "Nút chuyển đổi phụ đề",
  "caption_toggle_button_desc": "Hiển thị nút nổi trên các trang Google Meet để nhanh chóng ẩn/hiện phụ đề",
  "caption_toggle_enabled": "Đã bật nút chuyển đổi phụ đề",
  "caption_toggle_disabled": "Đã tắt nút chuyển đổi phụ đề",
  "captions_disabled_title": "Phụ đề đã tắt",
  "captions_disabled_body": "Tiện ích mở rộng này dựa vào phụ đề Google Meet để nhận nội dung cuộc họp. Phụ đề hiện đang tắt — phụ đề mới sẽ không được ghi lại.\n\nBật lại phụ đề để tiếp tục nhận bản chép lời.",
  "captions_disabled_reenable": "Bật lại phụ đề",
  "captions_disabled_dismiss": "Đã hiểu",

  // Sticker Note
  "sticker_note": "Ghi Chú Dán",
  "sticker_note_desc": "Bật ghi chú dán để ghi chú cuộc họp nhanh chóng",
  "sticker_enabled": "Đã bật ghi chú dán",
  "sticker_disabled": "Đã tắt ghi chú dán",
  "add_note": "Thêm Ghi Chú",

  // AI Service Messages
  "ai_meeting_content_intro": "Đây là nội dung cuộc họp trước đó: ",
  "ai_meeting_assistant_ready": "Tôi đã hiểu nội dung cuộc họp. Tôi có thể giúp gì cho bạn?",
  "ai_system_prompt_meeting": "You are a helpful assistant for meeting transcripts.",

  // Guide Animation
  "guide_animation_title": "📱 Cách Sử Dụng Tiện Ích Mở Rộng",
  "guide_animation_subtitle": "Theo dõi hướng dẫn hoạt hình để dễ dàng bắt đầu trải nghiệm phụ đề thông minh",
  "guide_step_1_title": "1. Tham Gia Cuộc Họp Google Meet",
  "guide_step_1_desc": "Mở Google Meet và tham gia bất kỳ cuộc họp nào",
  "guide_step_2_title": "2. Tìm Nút Phụ Đề",
  "guide_step_2_desc": "Tìm nút phụ đề (biểu tượng CC) trong thanh điều khiển dưới cùng của cuộc họp",
  "guide_step_3_title": "3. Bật Phụ Đề",
  "guide_step_3_desc": "Nhấp vào nút phụ đề và chọn \"Bật phụ đề\"",
  "guide_step_4_title": "4. Tiện Ích Mở Rộng Tự Động Kích Hoạt",
  "guide_step_4_desc": "Thanh bên của tiện ích mở rộng tự động kích hoạt và bắt đầu cung cấp các tính năng thông minh",
  "guide_step_5_title": "5. Tận Hưởng Các Tính Năng Thông Minh",
  "guide_step_5_desc": "Bây giờ bạn có thể sử dụng dịch thuật, tóm tắt, xuất và tất cả các tính năng khác!",
  "guide_start_demo": "Bắt Đầu Demo",
  "guide_restart": "Khởi Động Lại",

  // Cài đặt đồng bộ tự động
  "auto_sync_on_meeting_end": "Tự động đồng bộ khi kết thúc cuộc họp",
  "auto_sync_on_meeting_end_desc": "Tự động sao lưu khi bạn rời khỏi cuộc gọi Google Meet",
  "auto_sync_on_startup": "Tự động đồng bộ khi khởi động trình duyệt",
  "auto_sync_on_startup_desc": "Tự động đồng bộ lịch sử trò chuyện 5 ngày gần đây khi khởi động trình duyệt",

  // Google Drive - Bổ sung
  "backup_to_drive": "Sao lưu lên Drive",
  "backup_title": "Sao lưu",
  "backup_title_desc": "Sao lưu lịch sử trò chuyện của bạn lên Google Drive. Các tệp hiện có có thể được ghi đè hoặc bỏ qua.",

  // Google Drive - Phần khôi phục
  "restore_files": "Khôi phục tệp",
  "restore_files_desc": "Xem và quản lý các tệp sao lưu của bạn trong Google Drive",
  "view_details": "Xem chi tiết",

  // Google Drive - Danh sách tệp
  "backup_date": "Ngày sao lưu",
  "modified": "Đã sửa đổi",
  "actions": "Hành động",
  "view_chat_records": "Xem bản ghi trò chuyện",
  "restore_this_backup": "Khôi phục bản sao lưu này",
  "download_to_local": "Tải xuống cục bộ",
  "delete_this_backup": "Xóa bản sao lưu này",
  "delete_file": "Xóa tệp",
  "delete_file_confirm": "Bạn có chắc chắn muốn xóa \"{fileName}\" không?",
  "search_backups": "Tìm kiếm bản sao lưu...",
  "all_time": "Mọi thời điểm",
  "this_week": "Tuần này",
  "this_month": "Tháng này",
  "no_matching_backup_files": "Không tìm thấy tệp sao lưu phù hợp",
  "no_backup_files": "Không tìm thấy tệp sao lưu",
  "items_count": "{count} mục",

  // Google Drive - Xác thực
  "authenticating_with_google": "Đang xác thực với Google...",
  "authenticating_desc": "Vui lòng đợi trong khi chúng tôi kết nối an toàn với tài khoản Google của bạn",
  "logging_in": "Đang đăng nhập...",
  "login_with_google": "Đăng nhập bằng Google",

  // Chế độ xem lịch
  "chat_history_desc": "Xem và tìm kiếm lịch sử trò chuyện cuộc họp được sắp xếp theo ngày",
  "message": "tin nhắn",
  "messages": "tin nhắn",

  // Cài đặt AI
  "save_configuration": "Lưu cấu hình",

  // Cài đặt tiện ích mở rộng
  "extension_settings_desc": "Cấu hình các tính năng và hành vi cụ thể của tiện ích mở rộng",
  "select_date": "Chọn ngày",

  // Nhà cung cấp dịch thuật
  "translation_provider": "Nhà cung cấp dịch thuật",
  "translation_provider_desc": "Chọn nhà cung cấp dịch vụ dịch thuật",
  "provider_google": "Google Dịch",
  "provider_microsoft": "Microsoft Dịch",
  "provider_ai": "Dịch AI",
  "provider_google_desc": "Miễn phí, không cần cấu hình",
  "provider_microsoft_desc": "Miễn phí, không cần cấu hình",
  "provider_ai_desc": "Sử dụng dịch vụ AI đã cấu hình",
  "provider_deepl": "DeepL",
  "provider_deepl_desc": "Bản dịch chất lượng cao, yêu cầu khóa xác thực",
  "provider_local": "Dịch AI cục bộ",
  "provider_local_desc": "AI tích hợp Chrome, không cần internet",
  "local_translator_title": "Trình dịch AI cục bộ",
  "local_translator_checking": "Đang kiểm tra tính khả dụng...",
  "local_translator_not_supported": "Không được hỗ trợ trên thiết bị này",
  "local_translator_not_supported_hint": "Yêu cầu Chrome 138+ trên máy tính để bàn với GPU",
  "local_translator_downloading": "Đang tải xuống mô hình dịch...",
  "local_translator_download_progress": "Đã tải {percent}%",
  "local_translator_ready": "Mô hình sẵn sàng",
  "local_translator_test_title": "Kiểm tra dịch",
  "local_translator_input_placeholder": "Nhập văn bản cần dịch...",
  "local_translator_output_placeholder": "Bản dịch sẽ xuất hiện ở đây...",
  "local_translator_translate_btn": "Dịch",
  "local_translator_done": "Sử dụng AI cục bộ",
  "local_translator_source_lang": "Nguồn",
  "deepl_settings": "Cài đặt DeepL",
  "deepl_auth_key": "Khóa xác thực",
  "deepl_auth_key_placeholder": "Nhập khóa xác thực DeepL của bạn",
  "deepl_auth_key_help": "Lấy khóa của bạn tại deepl.com",
  "deepl_settings_saved": "Đã lưu cài đặt DeepL",
  "deepl_auth_key_required": "Khóa xác thực là bắt buộc",
  "save": "Lưu",
  "translation_provider_set": "Nhà cung cấp dịch thuật được đặt thành {provider}",
  // AI Settings - Provider UI
  "configuration_removed": "Đã xóa cấu hình",
  "active_service_changed": "Đã thay đổi dịch vụ mặc định",
  "optional": "Tùy chọn",
  "add_custom_provider": "Thêm nhà cung cấp tùy chỉnh",
  "add": "Thêm",
  "provider_name": "Tên nhà cung cấp",
  "api_base_url": "URL cơ sở API",
  "openai_compatible_hint": "Phải là endpoint tương thích OpenAI",
  "default_model": "Mô hình mặc định",
  "search_providers": "Tìm kiếm nhà cung cấp...",
  "select_provider_hint": "Chọn nhà cung cấp để cấu hình",
  "select_provider_desc": "Chọn từ danh sách bên trái",
  "set_as_default": "Đặt làm dịch vụ mặc định",
  "default_service_desc": "Sử dụng nhà cung cấp này cho tất cả tính năng AI",
  "configuration": "Cấu hình",
  "get_api_key": "Lấy khóa API",
  "model": "Mô hình",
  "select_model": "Chọn mô hình",
  "models_from_cache": "Danh sách mô hình từ bộ nhớ đệm · cập nhật mỗi 24h",
  "base_url": "URL cơ sở",
  "base_url_hint": "Ghi đè endpoint API mặc định (ví dụ: cho proxy)",
  "testing": "Đang kiểm tra kết nối...",
  "connection_success": "Kết nối thành công",
  "test_connection": "Kiểm tra kết nối",
  "remove": "Xóa",
  "category_international": "Quốc tế",
  "category_chinese": "Trung Quốc",
  "category_local": "Địa phương",
  "category_custom": "Tùy chỉnh",
  "active_badge": "Đang dùng",

  // Summary message UI
  "view_more": "Xem chi tiết",
  "view_less": "Ẩn chi tiết",
  "error_request_failed": "Yêu cầu thất bại",
  "error_status": "Lỗi",
  "time_just_now": "vừa xong",
  "time_minutes_ago": "{n} phút trước",
  "time_hours_ago": "{n} giờ trước",
  "error_expand": "Yêu cầu thất bại · nhấn để xem chi tiết",
  "error_collapse": "Ẩn chi tiết",
  "error_retry": "Thử lại",
  "error_details": "Thêm thông tin",
  "err_title_quota": "Đã hết hạn mức hôm nay",
  "err_desc_quota": "Bạn đã dùng hết hạn mức miễn phí hôm nay. Thử lại vào ngày mai hoặc nâng cấp gói của bạn.",
  "err_action_quota": "Xem gói cước",
  "err_title_auth": "API key không hợp lệ",
  "err_desc_auth": "API key của bạn có vẻ không đúng hoặc đã hết hạn. Vui lòng kiểm tra cài đặt.",
  "err_title_permission": "Truy cập bị từ chối",
  "err_desc_permission": "Tài khoản của bạn không có quyền sử dụng mô hình này. Kiểm tra gói cước của bạn.",
  "err_title_network": "Kết nối thất bại",
  "err_desc_network": "Không thể kết nối với dịch vụ AI. Vui lòng kiểm tra kết nối mạng.",
  "err_title_model": "Mô hình không khả dụng",
  "err_desc_model": "Không tìm thấy mô hình đã chọn. Hãy thử chuyển sang mô hình khác trong cài đặt.",
  "err_title_generic": "Đã xảy ra sự cố",
  "err_desc_generic": "Xảy ra lỗi không mong muốn. Vui lòng thử lại.",

  // Custom Prompts
  "custom_prompts": "Prompt tùy chỉnh",
  "custom_prompts_desc": "Tạo các mẫu prompt có thể tái sử dụng cho tóm tắt AI.",
  "custom_prompts_empty": "Chưa có prompt nào. Nhấn + để thêm.",
  "go_to_extension_settings": "Vào Cài đặt → Tiện ích mở rộng để thêm prompt.",
  "add_prompt": "Thêm prompt",
  "edit_prompt": "Chỉnh sửa prompt",
  "prompt_title": "Tiêu đề",
  "prompt_title_placeholder": "VD: Trích xuất từ vựng nâng cao",
  "prompt_content": "Nội dung prompt",
  "prompt_content_placeholder": "Nhập văn bản prompt…",
  "select_prompt": "Chọn prompt",
  "apply_prompts": "Áp dụng ({n} đã chọn)",
  "save": "Lưu",

  // AI & Translation Tab
  "tab_ai_translation": "Cài đặt AI",
  "tab_ai_translation_desc": "Nhà cung cấp AI, gợi ý tùy chỉnh và cài đặt dịch thuật",
  "tab_ai_models": "Mô hình dịch vụ",
  "tab_ai_prompts": "Thư viện prompt",
  "tab_translation_prefs": "Tùy chọn dịch thuật",
  "tab_general": "Chung",
  "tab_general_desc": "Ngôn ngữ, cỡ chữ, trợ giúp và quản lý dữ liệu",
  "tab_interface": "Giao diện",
  "tab_interface_desc": "Các tab bảng bên, nút phụ đề, lớp phủ cuộc họp và từ được đánh dấu",
  "tab_history": "Lịch",
  "tab_history_desc": "Lịch sử cuộc họp, trợ lý AI và xuất",
  "tab_cloud_sync": "Đám mây",
  "tab_cloud_sync_desc": "Sao lưu và khôi phục Google Drive",
  "meeting_history_tab": "Lịch sử cuộc họp",
  "ai_assistant_tab": "Trợ lý AI",
  "ai_chat_title": "Trợ lý AI cuộc họp",
  "ai_chat_empty_desc": "Chọn phạm vi ngày để bắt đầu cuộc trò chuyện dựa trên hồ sơ cuộc họp trong khoảng thời gian đó",
  "ai_reading_records": "Đang đọc hồ sơ cuộc họp",
  "ai_found_records": "Tìm thấy {count}",

  // Popup (additional)
  "extension_name": "Google Meet Caption Pro",
  "popup_subtitle": "Công cụ nâng cao Google Meet",
  "main_controls": "Điều khiển chính",
  "quick_actions": "Hành động nhanh",
  "recommended": "Được đề xuất",

  // AI Chat Panel - Date Range & Input
  "select_date_range": "Chọn phạm vi ngày họp",
  "preset_3d": "3 ngày gần đây",
  "preset_7d": "1 tuần gần đây",
  "preset_14d": "2 tuần gần đây",
  "preset_1m": "1 tháng gần đây",
  "preset_2m": "2 tháng gần đây",
  "preset_3m": "3 tháng gần đây",
  "preset_custom": "Phạm vi tùy chỉnh",
  "date_format_short": "D/M",
  "capsule_text": "{start} - {end} ({count} bản ghi)",
  "clear_chat": "Xóa cuộc trò chuyện",
  "no_ai_provider_before": "Chưa cấu hình dịch vụ AI, vui lòng",
  "no_ai_provider_link": "đến cài đặt AI",
  "no_ai_provider_after": "để cấu hình nhà cung cấp.",
  "placeholder_no_date": "Vui lòng chọn phạm vi ngày trước...",
  "placeholder_no_provider": "Vui lòng cấu hình dịch vụ AI trước...",
  "ai_input_placeholder": "Nhập câu hỏi, nhấn Cmd+Enter hoặc Ctrl+Enter để gửi nhanh",
  "select_ai_service": "Chọn dịch vụ AI",
  "default_label": "Mặc định",
  "current_provider": "Hiện tại: {name}",
  "select_ai": "Chọn AI",

  // Shared UI (General tab)
  "help_and_guide": "Trợ giúp & Hướng dẫn",
  "open_welcome_guide": "Mở hướng dẫn chào mừng",
  "ui_language_selector_desc": "Chọn ngôn ngữ giao diện tiện ích mở rộng",
  "data_management": "Quản lý dữ liệu",
  "highlight_words_section": "Tô sáng phụ đề",

  // Options tabs
  "tab_ai_models_nav": "Mô hình AI",
  "tab_visibility": "Khả năng hiển thị",
  "tab_translation": "Dịch thuật",
  "tab_translation_nav": "Dịch thuật",
  "tab_other": "Khác",
  "tab_calendar": "Google Calendar",
  "tab_data_sync": "Lịch sử cuộc họp",
  "tab_google_drive": "Google Drive",
  // Extension section
  "extension_section": "Tiện ích mở rộng",
  "enable_extension": "Bật tiện ích mở rộng",
  "extension_active_desc": "Chụp phụ đề đang hoạt động trên Google Meet",
  "extension_paused_desc": "Tiện ích mở rộng bị tạm dừng — không có phụ đề nào được chụp",
  "extension_toggle_footer": "Bật/tắt để tạm dừng hoặc tiếp tục chụp phụ đề mà không cần gỡ cài đặt tiện ích mở rộng.",

  // Popup tabs
  "popup_tab_meetings": "Cuộc họp",
  "popup_tab_live": "Đang diễn ra",
  "popup_tab_ai": "AI",
  "popup_tab_settings": "Cài đặt",
  "ai_no_content_title": "Chưa có nội dung cuộc họp",
  "ai_no_content_sub": "Tóm tắt cuộc họp và thông tin chi tiết AI sẽ xuất hiện ở đây sau các cuộc họp của bạn.",
  "ai_whats_coming": "Bạn sẽ nhận được gì",
  "ai_feature_summaries": "Tóm tắt do AI tạo",
  "ai_feature_highlights": "Điểm nổi bật và hành động cần thực hiện",
  "ai_feature_ask_meetings": "Hỏi bất cứ điều gì về cuộc họp của bạn",
  "ai_recent_meetings": "Cuộc họp gần đây",
  "ai_ask_ai_btn": "Hỏi AI",
  "ai_view_all_meetings": "Xem tất cả cuộc họp & tóm tắt",
  "meetings_connect_title": "Xem các cuộc họp Google Meet sắp tới",
  "meetings_connect_sub": "Kết nối Google Calendar để xem các cuộc họp sắp tới và nhận nhắc nhở.",
  "meetings_connect_note": "🔒 Chỉ quyền đọc. Dữ liệu của bạn là riêng tư.",
  "meetings_no_upcoming_title": "Không có cuộc họp sắp tới",
  "meetings_no_upcoming_sub": "Lịch của bạn trống. Hãy tận hưởng ngày của bạn!",
  "meetings_open_calendar_settings": "Mở cài đặt lịch",
  "meetings_today_label": "Hôm nay · {date}",
  "meetings_synced_now": "Vừa đồng bộ",
  "meetings_sync_ago": "Đã đồng bộ {time} trước",
  "meetings_sync_h": "g",
  "meetings_sync_m": "ph",
  "meetings_sync_s": "gi",
  "meetings_in_progress": "Đang diễn ra",
  "meetings_starts_in": "Bắt đầu sau {min} phút",
  "meetings_untitled": "Cuộc họp không có tiêu đề",
  "meetings_participants": "{n} người tham gia",
  "meetings_join_btn": "Tham gia cuộc họp",
  "meetings_later_today": "Sau đó hôm nay",
  "meetings_view_schedule": "Xem lịch trình đầy đủ",
  "live_no_meeting_title": "Không phát hiện cuộc họp trực tiếp",
  "live_no_meeting_sub": "Tham gia cuộc họp Google Meet để xem phụ đề trực tiếp và trạng thái dịch thuật.",
  "live_how_it_works": "Cách hoạt động",
  "live_how_body": "Chúng tôi sẽ tự động phát hiện khi bạn đang trong Google Meet và bắt đầu ghi chú phụ đề.",
  "live_meet_detected": "Đã phát hiện Google Meet",
  "live_now": "Đang phát trực tiếp",
  "live_captions_label": "Phụ đề",
  "live_active": "Đang hoạt động",
  "live_translation_label": "Dịch thuật",
  "live_translation_active": "Đang hoạt động (EN → {lang})",
  "live_off": "Tắt",
  "live_open_panel_btn": "Mở bảng trực tiếp",
  "live_hint": "Quản lý dịch thuật trong Cài đặt",
  "live_manage_settings": "Quản lý trong Cài đặt ›",

  // Settings tab
  "settings_open_full": "Mở cài đặt đầy đủ",
  "settings_connected": "Đã kết nối",
  "settings_not_connected": "Chưa kết nối",
  "settings_sync_range": "Phạm vi đồng bộ",

  // Google Calendar page
  "calendar_page_title": "Lịch",
  "calendar_page_desc": "Đồng bộ Google Calendar để nhận lịch họp và thông báo nhắc nhở",
  "calendar_connection_status": "Trạng thái kết nối Google Calendar",
  "calendar_not_connected": "Chưa kết nối",
  "calendar_not_connected_desc": "Kết nối Google Calendar để xem các cuộc họp sắp tới",
  "calendar_connected": "Đã kết nối",
  "calendar_connect_button": "Kết nối Google Calendar",
  "calendar_disconnect_button": "Ngắt kết nối",
  "calendar_privacy_note": "Chúng tôi chỉ đọc sự kiện trên lịch của bạn. Chúng tôi sẽ không sửa đổi, tạo hoặc xóa bất kỳ nội dung nào",
  "calendar_learn_more": "Tìm hiểu thêm",
  "calendar_sync_settings": "Cài đặt đồng bộ",
  "calendar_auto_sync": "Tự động đồng bộ",
  "calendar_auto_sync_desc": "Đồng bộ định kỳ các cuộc họp mới nhất từ Google Calendar",
  "calendar_sync_frequency": "Tần suất đồng bộ",
  "calendar_sync_frequency_desc": "Tự động đồng bộ theo khoảng thời gian đã đặt",
  "calendar_sync_range": "Phạm vi đồng bộ",
  "calendar_sync_range_desc": "Chỉ đồng bộ các cuộc họp trong phạm vi tương lai được chỉ định",
  "calendar_sync_content": "Nội dung cần đồng bộ",
  "calendar_content_title": "Tiêu đề cuộc họp",
  "calendar_content_time": "Giờ bắt đầu và kết thúc",
  "calendar_content_link": "Liên kết Google Meet",
  "calendar_content_attendees": "Thông tin người tham dự",
  "calendar_content_location": "Địa điểm cuộc họp",
  "calendar_tip": "Mẹo: Sau khi kết nối, bạn sẽ nhận được thông báo hệ thống trước các cuộc họp sắp tới",
  "calendar_frequency_15min": "15 phút",
  "calendar_frequency_30min": "30 phút",
  "calendar_frequency_1hour": "1 giờ",
  "calendar_frequency_2hours": "2 giờ",
  "calendar_range_7days": "7 ngày tới",
  "calendar_range_14days": "14 ngày tới",
  "calendar_range_30days": "30 ngày tới",
  "calendar_range_60days": "60 ngày tới",
  "calendar_reminders": "Nhắc nhở cuộc họp",
  "calendar_reminders_enable": "Bật nhắc nhở cuộc họp",
  "calendar_reminders_enable_desc": "Gửi thông báo hệ thống trước khi cuộc họp bắt đầu",
  "calendar_reminders_advance": "Thời gian nhắc nhở trước",
  "calendar_reminders_advance_desc": "Gửi thông báo bao nhiêu phút trước cuộc họp",
  "calendar_reminder_on_time": "Khi đến giờ họp",
  "calendar_reminder_5min": "5 phút trước",
  "calendar_reminder_10min": "10 phút trước",
  "calendar_reminder_15min": "15 phút trước",
  "calendar_reminder_30min": "30 phút trước",
  "calendar_test_notif": "Thông báo thử nghiệm",
  "calendar_test_notif_desc": "Gửi ngay một nhắc nhở thử nghiệm để xác nhận thông báo hoạt động",
  "calendar_test_notif_btn": "Gửi thử nghiệm",
  "calendar_test_notif_success": "Đã gửi thông báo thử nghiệm, kiểm tra thông báo hệ thống của bạn",
  "calendar_test_notif_fail": "Gửi thất bại, vui lòng xác nhận đã cấp quyền thông báo",
  "calendar_sync_status": "Trạng thái đồng bộ",
  "calendar_last_sync": "Lần đồng bộ cuối: {time}",
  "calendar_sync_now": "Đồng bộ ngay",
  "calendar_syncing": "Đang đồng bộ...",
  "calendar_sync_success": "Đồng bộ thành công, đã nhận <strong>{n}</strong> sự kiện lịch",
  "calendar_sync_error_default": "Đồng bộ thất bại, vui lòng thử lại",
  "calendar_syncing_msg": "Đang lấy sự kiện từ Google Calendar...",
  "calendar_event_no_title": "(Không có tiêu đề)",
  "calendar_event_attendees": "{n} người tham dự",
  "calendar_meet_link": "Liên kết Meet",
  "calendar_event_more": "{n} sự kiện nữa",
  "calendar_sync_empty": "Nhấp Đồng bộ ngay để nhận các cuộc họp sắp tới",
  "settings_local_data": "Dữ liệu cục bộ",
  "settings_local_days": "{n} ngày",

  "version_label": "v",
};
